...
function DashboardContent({ liveData }: { liveData: LiveDashboardData }) {
  ...
  return <div className="app-shell simple-dashboard">
    ...
    <main id="main" className="main">
      ...
      {view !== "overview" && view !== "about" ? <MeasurementGuide /> : null}
...
function AudienceView({ rows, onOpen, asOf }: { rows: Mailing[]; onOpen: (id: string) => void; asOf: string }) {
  ...
  return <div className="stack newsletter-workspace">
    ...
    <section className="panel table-panel"><PanelHeader eyebrow="Målgruppefordelt indholdsresultat" title={`Hvad klikkede ${selectedAudience} på?`} meta={`${linkCoverage} af ${series.length} udgaver har målgruppefordelte linkklik`} /><p className="content-scope-note">Visningen dokumenterer målgruppens klik. Den dokumenterer ikke, at indholdet udelukkende blev sendt til denne målgruppe.</p><div className="table-scroll"><table><thead><tr><th>Nyhed eller link</th><th>Emne</th><th>Udsendelse</th><th>Registrerede linkklik</th><th>Klik pr. 100 modtagere</th></tr></thead><tbody>{links.length ? visibleLinks.map((item) => <tr key={`${item.mailing.id}-${item.destination}`}><td><ContentCell item={item} /></td><td><TopicBadge item={item} /></td><td>{item.mailing.date}</td><td><LinkClicks item={item} /></td><td><LinkRate item={item} /></td></tr>) : <EmptyRow columns={5} text="Ingen dokumenterede linkresultater for denne målgruppe i den valgte periode." />}</tbody></table></div>{links.length > 8 ? <ExpandButton expanded={linksExpanded} total={links.length} onClick={() => setLinksExpanded((value) => !value)} /> : null}</section>
  </div>;
}
...
function MailingView({ rows, selected, onChange, asOf }: { rows: Mailing[]; selected?: Mailing; onChange: (id: string) => void; asOf: string }) {
  ...
  return <div className="stack">
    ...
    <section className="panel table-panel"><PanelHeader eyebrow="Samlet resultat for udsendelsen" title="Hvad blev der klikket på i hele udsendelsen?" meta={coverageFor(selected).linkPerformance ? "Klikmål angives ved hver række" : "Linkdata ikke tilgængelige"} /><p className="content-scope-note">Tallene omfatter hele udsendelsen og kan derfor ikke tilskrives en bestemt målgruppe.</p><div className="table-scroll"><table><thead><tr><th>Nyhed eller link</th><th>Emne</th><th>Registrerede linkklik</th><th>Klik pr. 100 leverede</th></tr></thead><tbody>{content.length ? (expanded ? content : content.slice(0, 8)).map((item, index) => <tr key={`${item.destination}-${index}`}><td><ContentCell item={item} /></td><td><TopicBadge item={item} /></td><td><LinkClicks item={item} /></td><td><LinkRate item={item} /></td></tr>) : <EmptyRow columns={4} text="Ingen dokumenterede linkresultater for denne udsendelse." />}</tbody></table></div></section>
