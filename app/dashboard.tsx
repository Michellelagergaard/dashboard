...
function AudienceView({ rows, onOpen }: { rows: Mailing[]; onOpen: (id: string) => void }) {
  ...
  useEffect(() => {
    if (!availableSegments.includes(audience) && availableSegments.length) setAudience(availableSegments[0]);
  }, [audience, availableSegments]);
...
function MailingView({ rows, selected, onChange }: { rows: Mailing[]; selected?: Mailing; onChange: (id: string) => void }) {
  ...
  return <div className="stack">
    ...
    <section className="panel table-panel"><PanelHeader eyebrow="Indhold" title="Hvad blev der klikket på?" meta={coverageFor(selected).linkPerformance ? "Dokumenterede unikke klik" : "Linkdata ikke tilgængelige"} /><div className="table-scroll"><table><thead><tr><th>Nyhed eller link</th><th>Unikke klik</th><th>Andel af leverede</th></tr></thead><tbody>{content.length ? content.map((item) => <tr key={item.destination}><td><ContentCell item={item} /></td><td><b>{num(item.clicks)}</b></td><td>{pct(item.rate)}</td></tr>) : <EmptyRow columns={3} text="Der er ingen offentliggørelsesklare klik pr. link for denne udsendelse." />}</tbody></table></div></section>
    ...
    <section className="panel table-panel"><PanelHeader eyebrow="Målgruppe × indhold" title="Hvad reagerede målgrupperne på?" meta={coverageFor(selected).segmentLinkPerformance ? "Dokumenterede unikke klik" : "Målgruppefordelte linkklik ikke tilgængelige"} /><div className="table-scroll"><table><thead><tr><th>Nyhed eller link</th><th>Målgruppe</th><th>Unikke klik</th><th>Andel af målgruppens modtagere</th></tr></thead><tbody>{segmentLinks.length ? segmentLinks.map((item) => <tr key={`${item.destination}-${item.audience}`}><td><ContentCell item={item} /></td><td>{item.audience}</td><td><b>{num(item.clicks)}</b></td><td>{pct(item.rate)}</td></tr>) : <EmptyRow columns={4} text="Der er ingen offentliggørelsesklare segmentfordelte linkklik for denne udsendelse." />}</tbody></table></div></section>
  </div>;
}
