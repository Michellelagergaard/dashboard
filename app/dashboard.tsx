...
function MailingView({ rows, selected, onChange }: { rows: Mailing[]; selected?: Mailing; onChange: (id: string) => void }) {
  ...
  return <div className="stack">
    ...
    <section className="panel table-panel"><PanelHeader eyebrow="Målgrupper" title="Hvordan reagerede målgrupperne?" meta={`${memberSegmentFieldLabel} · grupper under ${minimumPublicSegmentSize} skjules`} /><div className="table-scroll"><table><thead><tr><th>Segment</th><th>Modtagere</th><th>Åbnet</th><th>Klikket</th><th>CTOR</th><th>Afmeldinger</th></tr></thead><tbody>{performance.length ? performance.map((item) => <tr key={item.name}><td><strong>{item.name}</strong></td><td>{item.recipientsLabel}</td><td>{pct(item.openRate)}</td><td><b>{pct(item.clickRate)}</b></td><td>{pct(item.ctor)}</td><td>{item.unsubscribes === null ? "—" : num(item.unsubscribes)}</td></tr>) : <EmptyRow columns={6} text="Segmentresultater er ikke klar for denne udsendelse." />}</tbody></table></div></section>
