...
function Overview({ rows, onOpen }: { rows: Mailing[]; onOpen: (id: string) => void }) {
  ...
  return <div className="stack">
    ...
    <section className="panel table-panel"><PanelHeader eyebrow="Seneste udsendelse" title="Mest klikkede links" meta="Unikke klik · mindst 5" /><div className="table-scroll"><table><thead><tr><th>Destination</th><th>Unikke klik</th><th>Andel af leverede</th></tr></thead><tbody>{topLinks.length ? topLinks.map((item) => <tr key={item.destination}><td><strong>{shortLink(item.destination)}</strong><span>{item.destination}</span></td><td><b>{num(item.clicks)}</b></td><td>{pct(item.rate)}</td></tr>) : <EmptyRow columns={3} text="Ungapped leverede ikke brugbare klik pr. link for denne udsendelse." />}</tbody></table></div></section>
...
function AudienceView({ rows, onOpen }: { rows: Mailing[]; onOpen: (id: string) => void }) {
  ...
  return <div className="stack newsletter-workspace">
    ...
    <section className="panel table-panel"><PanelHeader eyebrow="Indholdsperformance" title={`Hvad klikkede ${audience} på?`} meta="Kun dokumenterede segmentklik" /><div className="table-scroll"><table><thead><tr><th>Destination</th><th>Udsendelse</th><th>Unikke klik</th><th>Andel af segmentet</th></tr></thead><tbody>{links.length ? links.slice(0, 20).map((item) => <tr key={`${item.mailing.id}-${item.destination}`}><td><strong>{shortLink(item.destination)}</strong><span>{item.destination}</span></td><td>{item.mailing.date}</td><td><b>{num(item.clicks)}</b></td><td>{pct(item.rate)}</td></tr>) : <EmptyRow columns={4} text="API'et har endnu ikke leveret verificerbare segmentfordelte linkklik for denne målgruppe." />}</tbody></table></div></section>
  </div>;
}
...
function MailingView({ rows, selected, onChange }: { rows: Mailing[]; selected?: Mailing; onChange: (id: string) => void }) {
  ...
  return <div className="stack">
    ...
    <section className="panel table-panel"><PanelHeader eyebrow="Målgrupper" title="Hvordan reagerede målgrupperne?" meta={`${memberSegmentFieldLabel} · grupper under ${minimumPublicSegmentSize} skjules`} /><div className="table-scroll"><table><thead><tr><th>Målgruppe</th><th>Dokumenteret emnefelt</th><th>Modtagere</th><th>Åbnet</th><th>Klikket</th><th>CTOR</th><th>Afmeldinger</th></tr></thead><tbody>{performance.length ? performance.map((item) => <tr key={item.name}><td><strong>{item.name}</strong></td><td>{subjectForAudience(selected, item.name) || <span className="muted-cell">Ikke tilgængeligt i API-udtrækket</span>}</td><td>{item.recipientsLabel}</td><td>{pct(item.openRate)}</td><td><b>{pct(item.clickRate)}</b></td><td>{pct(item.ctor)}</td><td>{item.unsubscribes === null ? "—" : num(item.unsubscribes)}</td></tr>) : <EmptyRow columns={7} text="API'et har ikke leveret verificerbare målgrupperesultater for denne udsendelse." />}</tbody></table></div></section>
