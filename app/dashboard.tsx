...
import type { ClickMeasurement, LiveDashboardData, LiveMailing } from "./live-data";
...
function Overview({ rows, onOpen }: { rows: Mailing[]; onOpen: (id: string) => void }) {
  ...
  return <div className="stack">
    ...
    <section className="panel trend-panel"><PanelHeader eyebrow="Udvikling" title="Klik og åbninger over tid" meta="Ét punkt pr. udsendelse" /><Trend rows={rows} /></section>
...
function AudienceView({ rows, onOpen }: { rows: Mailing[]; onOpen: (id: string) => void }) {
  ...
  const normal = summarizeSegmentRates(normalSeries.map(item => item.data));
  const normalOpen = normal.open ?? 0;
  const normalClick = normal.click ?? 0;
  const normalNote = `${normal.count} foregående udgaver · ${normal.method === "delivered" ? "vægtet efter leveringer" : "uvægtet gennemsnit"}`;
  ...
  return <div className="stack newsletter-workspace">
    ...
    <section className="panel table-panel"><PanelHeader eyebrow="Målgruppefordelt indholdsresultat" title={`Hvad klikkede ${selectedAudience} på?`} meta={`${linkCoverage} af ${series.length} udgaver har målgruppefordelte linkklik`} /><p className="content-scope-note">Visningen dokumenterer målgruppens klik. Den dokumenterer ikke, at indholdet udelukkende blev sendt til denne målgruppe.</p><div className="table-scroll"><table><thead><tr><th>Nyhed eller link</th><th>Emne</th><th>Udsendelse</th><th>Registrerede linkklik</th><th>Klik pr. 100 modtagere</th></tr></thead><tbody>{links.length ? visibleLinks.map((item) => <tr key={`${item.mailing.id}-${item.destination}`}><td><ContentCell item={item} /></td><td><TopicBadge item={item} /></td><td>{item.mailing.date}</td><td><LinkClicks item={item} /></td><td><LinkRate item={item} /></td></tr>) : <EmptyRow columns={5} text="API'et har endnu ikke leveret verificerbare segmentfordelte linkklik for denne målgruppe." />}</tbody></table></div>{links.length > 8 ? <ExpandButton expanded={linksExpanded} total={links.length} onClick={() => setLinksExpanded((value) => !value)} /> : null}</section>
  </div>;
}
...
function EditorialIntelligence({ rows }: { rows: Mailing[] }) {
  const coveredRows = rows.map(row => ({ ...row, content: row.content.filter(isComparableLink) })).filter(row => row.content.length);
